...
ORIGINAL_VIDEO_FILE = os.path.abspath(
    BATCH_CONFIG['SETTINGS']['original_video_file_path'].strip("\""))
...
def trim_clip(input_sound: AudioSegment) -> AudioSegment:
    ...
    """
    trim_leading_silence: AudioSegment = lambda x: x[detect_leading_silence(
        x):]
...
def get_speed_factor(audio: AudioSegment, desired_duration: float) -> float:
    ...
    """
    raw_duration: AudioSegment = AudioSegment.from_file(
        audio, format="wav").duration_seconds
...
def stretch_audio(audio_file_to_stretch: Any, speed_factor: float,) -> AudioSegment:
    ...
    stretched_audio = pyrubberband.time_stretch(
        y=data, sr=sample_rate, rate=speed_factor, rbargs={'--fine': '--fine'})
    # soundfile.write(f'{workingFolder}\\temp_stretched.wav', streched_audio, sampleRate)
    soundfile.write(virtual_temp_audio_file, stretched_audio,
...
def generate_virtual_audio_file(path: str, file_format: str = "mp3", frame_rate: int = NATIVE_SAMPLE_RATE) -> bytes:
    ...
    Returns:
        bytes: Virtual file 
    """
    clip: AudioSegment = trim_clip(AudioSegment.from_file(
        file=path, format=file_format, frame_rate=frame_rate))
...
def generate_filename(lang_dict: dict, original_video_file: str, output_folder: str) -> str:
    """Use video file name to use in the name of the output file
       Add language name and language code
    ...
    lang_name = langcodes.get(lang_dict['languageCode']).get(
        lang.to_alpha3()).display_name()
...
def build_audio(all_clips: dict, lang_dict: dict, total_length: int, use_two_pass: bool) -> AudioSegment:
    ...
    for key, value in all_clips.items():
        value['TTS_FilePath_Trimmed'] = os.path.join(
            WORKING_FOLDER, f"{key}_t.wav")
        ...
        if use_two_pass:
            ...
            if FORCE_STRETCH:
                value['speed_factor'] = get_speed_factor(
                    clip, value['duration_ms'])
                clip = stretch_audio(clip, value['speed_factor'])
                print(
                    f"  Calculated Speed Factor (2nd Pass): {count} of {len(all_clips)}")
            else:
